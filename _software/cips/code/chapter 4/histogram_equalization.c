...
void perform_histogram_equalization(unsigned char **image, 
...
int main(void)
{
    ...
    perform_histogram_equalization(in_img, histogram, 256, 256, width, height);

    unsigned long histogram2[256]={0};
    calculate_histogram(in_img, histogram2, width, height);

    int height2=100;
    out_img = allocate_image_array(width, height2);

    draw_histogram(out_img, histogram2, width, height2, 255);
